"""
Test Movement Detection with your own data
Copy the working functions and test with custom datasets
"""
...
# Copy of working movement detection functions
...
def test_your_custom_data():
    """Test with your own financial data."""
    
    print("🧪 Testing Movement Detection with Custom Data")
    print("=" * 50)
    
    # EXAMPLE 1: Replace this with your actual data
    your_data = pd.DataFrame({
        ...
        'account': [
            'Your Revenue Account', 'Your Revenue Account', 'Your Revenue Account', 'Your Revenue Account', 'Your Revenue Account',
            'Your Revenue Account', 'Your Revenue Account', 'Your Revenue Account', 'Your Revenue Account', 'Your Revenue Account'
        ],
        'amount': [
            150000, 160000, 180000, 170000, 190000,  # 2023 data
            180000, 200000, 220000, 210000, 240000   # 2024 data (20%, 25%, 22%, 24%, 26% increases)
        ]
    })
    ...
    print("\n1. Monthly Summaries...")
    monthly_summary, monthly_stats = calculate_monthly_summaries(your_data)
    ...
    if monthly_summary is not None:
        ...
        if mom_stats['success']:
            ...
            if threshold_stats['success'] and significant_movements is not None:
                print(f"\n📈 Significant Movements (>10%):")
                for i, row in significant_movements.iterrows():
                    print(f"  - {row['account']}: {row['percentage_change']:+.1f}% ({row['significance']}) ${row['absolute_change']:+,.0f}")
    
    print(f"\n🎯 How to customize this for your data:")
    print(f"1. Replace 'your_data' DataFrame with your actual financial data")
    print(f"2. Ensure columns are named: 'date', 'account', 'amount'")
    print(f"3. Date format: YYYY-MM-DD (like '2024-01-31')")
    print(f"4. Run: python3 test_your_data.py")

if __name__ == '__main__':
    test_your_custom_data()